...
const before = await p.$$eval("#m mark.oga-hl", els=>els.map(e=>e.textContent.trim()));
console.log("while the modal is hidden :", JSON.stringify(before));
await p.waitForTimeout(2500);   // the reveal fires at 2.5s
const after = await p.$$eval("#m mark.oga-hl", els=>els.map(e=>
...
if (before.length !== 2 || after.length !== 2 || !visible) {
  console.error("FAIL: modal content was not marked");
  process.exitCode = 1;
} else {
